...
def test_parse_term():
    ...
    # Added tests for modulo
    tokens = tokenize("8%4")
    ast, tokens = parse_term(tokens)
    assert ast == {'tag': '%', 'left': {'tag': 'number', 'value': 8}, 'right': {'tag': 'number', 'value': 4}}

    tokens = tokenize("9%3*2")
    ast, tokens = parse_term(tokens)
    assert ast == {'tag': '*', 'left': {'tag': '%', 'left': {'tag': 'number', 'value': 9}, 'right': {'tag': 'number', 'value': 3}}, 'right': {'tag': 'number', 'value': 2}}
...
def test_parse_statement():
    ...
    # Added tests for modulo
    tokens = tokenize("3+(6+5)%8")
    ast, tokens = parse_statement(tokens)
    assert ast == { 'tag': '+', 'left': {'tag': 'number', 'value': 3}, 'right': {'tag': '%', 'left': {'tag': '+', 'left': {'tag': 'number', 'value': 6}, 'right': {'tag': 'number', 'value': 5}}, 'right': {'tag': 'number', 'value': 8}}}


    tokens = tokenize("print 7%9")
    ast, tokens = parse_statement(tokens)
    assert ast == {'tag': 'print', 'value': {'tag': '%', 'left': {'tag': 'number', 'value': 7}, 'right': {'tag': 'number', 'value': 9}}}
