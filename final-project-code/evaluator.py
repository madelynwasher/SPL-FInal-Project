...
def evaluate(ast):
    ...
    if ast["tag"] == "number":
        return ast["value"]
    if ast["tag"] in ["+", "-", "*", "/", "%"]: # Added modulo to list
...
# Added modulo evaluation test function
def test_evaluate_modulo():
    print("testing evaluate modulo")
    ast = {
        "tag": "%",
        "left": {"tag": "number", "value": 7},
        "right": {"tag": "number", "value": 3}
    }
    assert evaluate(ast) == 1
...
if __name__ == "__main__":
    ...
    test_evaluate_modulo() # Added to test modulo functionality
